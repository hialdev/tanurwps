...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Stages</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Semua stage</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
        ...
        <div style="aspect-ratio:1/1; width:3em; height:3em"
            ...
            {{ count($stages) }}</div>
        <a href="{{ route('stage.add') }}" class="btn btn-primary btn-al-primary"><i class="ti ti-plus me-2"></i>Tambah</a>
    </div>
    <div>
        <form action="{{ url()->current() }}" method="GET">
            <div class="row align-items-end mb-3 flex-wrap">
                <div class="col-md-4 mb-2">
                    ...
                    <input type="text" class="form-control" placeholder="Cari Stage" name="search"
                        value="{{ $filter->q ?? '' }}">
                </div>
                <div class="col-md-3 mb-2">
                    <label for="field" class="form-label">Urutkan Berdasarkan</label>
                    <select name="field" id="field" class="form-select">
                        @foreach (getModelAttributes('Stage', []) as $atr)
        ...
        <table class="table border text-nowrap mb-0 align-middle">
            <thead class="text-dark fs-4">
                <tr>
                    ...
                    <th>
                        <h6 class="fs-4 fw-semibold mb-0">Tasks</h6>
                    </th>
                    <th>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div>
                            <div class="badge bg-primary-subtle text-primary fw-semibold fs-2 mb-1" style="">Urutan ke {{ $stage->order ?? 'belum diatur' }}</div>
                            <h6 class="fw-semibold text-dark mb-1" style="">{{ $stage->name }}</h6>
                        </div>
                    </td>
                    <td>
                        <div>
                            <div class="fw-normal fs-2 text-muted" style="white-space:normal;">Deadline
                            </div>
                            <h6 class="fw-semibold text-secondary mb-1 fs-2" style="">{{ $stage->deadline_days }} hari</h6>
                        </div>
                        <div>
                            <div class="fw-normal fs-2 text-muted" style="white-space:normal;">Description
                            </div>
                            <div class="text-dark fs-2 mb-1" style="white-space:normal;">{{ $stage->description ?? 'tidak ada deskripsi' }}</div>
                        </div>
                    </td>
                    <td>
                        <button type="button"
                            class="dropdown-item fs-2 text-center d-inline-flex p-2 px-3 align-items-center gap-2 bg-secondary text-white rounded-3"
                            data-bs-toggle="modal" data-bs-target="#attachmentModal-{{$stage->id}}"><i
                                class="fs-4 ti ti-file"></i> {{ $stage->attachments->count() }} File</button>

                        <!-- List Product modal -->
                        <div class="modal fade " id="attachmentModal-{{$stage->id}}" tabindex="-1"
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body pt-0">
                                        @forelse ($stage->attachments as $attachment)
                    ...
                    <td>
                        <button type="button"
                            class="dropdown-item fs-2 text-center d-inline-flex p-2 px-3 align-items-center gap-2 bg-primary text-white rounded-3"
                            data-bs-toggle="modal" data-bs-target="#taskModal-{{$stage->id}}"><i
                                class="fs-4 ti ti-checklist"></i> {{ $stage->tasks->count() }} Task</button>

                        <!-- List Product modal -->
                        <div class="modal fade " id="taskModal-{{$stage->id}}" tabindex="-1"
                            aria-labelledby="vertical-center-modal" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header d-flex align-items-center">
                                    ...
                                    <div class="modal-body pt-0">
                                        @forelse ($stage->tasks as $task)
                                            <div class="p-2 border border-dashed rounded-3 mb-2">
                                                <div>
                                                    <div class="badge bg-primary-subtle text-primary fw-semibold fs-2 mb-1" style="">Urutan ke {{ $task->order ?? 'belum diatur' }}</div>
                                                    <div class="fw-normal fs-2 text-muted" style="white-space:normal;">Nama task
                                                    </div>
                                                    <h6 class="fw-semibold text-dark mb-1 fs-3" style="">{{ $task->name }}</h6>
                                                </div>
                                                <div>
                                                    <div class="fw-normal fs-2 text-muted" style="white-space:normal;">Score Point
                                                    </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-dark fs-2 mb-1" style="white-space:normal;">{{ $task->description ?? 'tidak ada deskripsi' }}</div>
                                                </div>
                                            </div>
                                        @empty
                    ...
                    <td>
                        <div class="dropdown dropstart">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <li>
                                    <a href="{{route('stage.edit', $stage->id)}}" class="dropdown-item d-flex align-items-center gap-3"><i
                                            class="fs-4 ti ti-edit"></i>Edit</a>
                                </li>
                                <li>
                                    <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                        data-bs-toggle="modal" data-bs-target="#deleteModal-{{$stage->id}}"><i
                        ...
                        <!-- Delete Modal -->
                        <div id="deleteModal-{{$stage->id}}" class="modal fade" tabindex="-1"
                                ...
                                <div class="modal-content p-3 modal-filled bg-danger">
                                    <div class="modal-header modal-colored-header text-white">
                                        <h4 class="modal-title text-white" id="danger-header-modalLabel">
                                            Yakin ingin menghapus stage ?
                                        </h4>
                                        <button type="button" class="btn-close btn-close-white"
                                            data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body" style="width: fit-content; white-space:normal">
                                        <h5 class="mt-0 text-white">Stage "{{$stage->name}}" akan dihapus</h5>
                                        <p class="text-white">Segala data yang berkaitan dengan stage tersebut juga akan dihapus secara permanen.</p>
                                    </div>
...
$(document).ready(function () {
    $('.delete-attachment-btn').on('click', function () {
        ...
        // Update form action
        $('#deleteAttachmentForm').attr('action', '/stages/' + stageId + '/attachments/' + attachmentId);
    });
});
</script>
