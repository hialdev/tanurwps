...
<div class="p-4 px-3 bg-tanur-green pb-4">
    <div class="d-block text-decoration-none text-dark mb-2 text-white rounded-3 position-relative">
        <div class="badge bg-tanur-coklat text-white fs-2 fw-semibold mb-2">{{ $approval->workspace->code }}</div>
        <div class="d-flex align-items-center gap-2 position-absolute top-0 end-0 m-2 ">
            <div class="bg-{{$approval->workspace->getStatus()['color']}} rounded-3 p-1 px-2 text-white fs-2 fw-semibold">
        ...
        <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <div>
              ...
              <div class="fs-2" style="white-space: nowrap">{{ $approval->workspace->requester->level }}</div>
            </div>
        </div>

        <div class="fs-3 fw-semibold">{{$approval->workspace->name}}</div>
        <div class="fs-2">{{$approval->workspace->description ?? 'tidak ada deskripsi'}}</div>
        <div class="fs-2 mt-1 text-capitalize"><i class="ti fs-3 ti-package me-2"></i> {{$approval->workspace->product_type}}</div>
        <div class="fs-2 mt-1"><i class="ti fs-3 ti-map-pin me-2"></i> {{ $approval->workspace->city.'. '.$approval->workspace->postal_code}}</div>
        <div class="fs-2 mt-1"><i class="ti fs-3 ti-user-circle me-2"></i> {{$approval->workspace->pic_name}}</div>
        <div class="fs-2 mt-1"><i class="ti fs-3 ti-phone me-2"></i> {{$approval->workspace->pic_phone}}</div>
        <div class="fs-2 mt-1"><i class="ti fs-3 ti-mail me-2"></i> {{$approval->workspace->pic_email}}</div>
        <div class="d-flex align-items-start flex-column gap-2 mt-2">
            <div class="btn btn-light btn-sm rounded-pill"><i class="ti fs-3 ti-user-circle me-1"></i> {{$approval->workspace->total_pilgrim_male+$approval->workspace->total_pilgrim_female}} Jamaah <span class="text-secondary mx-1"><i class="ti ti-gender-male me-1"></i>{{$approval->workspace->total_pilgrim_male}}</span> <span class="text-danger"><i class="ti ti-gender-female me-1"></i>{{$approval->workspace->total_pilgrim_female}}</span></div>
            @if($approval->workspace->is_approved)
                <div class="fs-2"><i class="ti fs-3 ti-timeline-event me-1"></i> <span class="text-white fw-semibold">{{ $approval->workspace->stageAnalytic()->finished }}</span> / {{ $approval->workspace->stageAnalytic()->total }} Stage</div>
                <div class="fs-2"><i class="ti fs-3 ti-subtask me-1"></i> <span class="text-white fw-semibold">{{ $approval->workspace->taskAnalytic()->finished }}</span> / {{ $approval->workspace->taskAnalytic()->total }} Task</div>
            @endif
        </div>
        ...
        <div class="d-flex text-{{$approval->workspace->getStatus()['color']}} align-items-center mt-2 fw-semibold gap-2">
            ...
            <div class="fs-2 text-white">{{$approval->workspace->getStatus()['message']}}</div>
        </div>
        @endif
            ...
            @foreach($approval->workspace->approvers as $approver)
                <div class="p-3 shadow-sm bg-white rounded-3">
                    ...
                    <div class="d-flex mt-2 align-items-center justify-content-between gap-2">
                        <div class="badge fs-2 rounded-3 fw-semibold text-{{$approval->workspace->approver_status[$approver->id]['color']}} bg-{{$approval->workspace->approver_status[$approver->id]['color']}}-subtle">{{ $approval->workspace->approver_status[$approver->id]['name'] }}</div>
                        ...
                        <a target="_blank" href="https://api.whatsapp.com/send?phone={{ $approver->person['phone'] }}&text=Mohon memberikan tanggapan terhadap Approval workspace saya ({{ $approval->requester->person['name'] }}) dengan kode {{ $approval->workspace->code }}" class="btn btn-light rounded-circle btn-sm fs-4"><i class="ti ti-brand-whatsapp"></i></a>
                    </div>
